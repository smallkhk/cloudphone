    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            ...
            @if ($order->phoneNumbers->isNotEmpty())
                <div class="card">
                    <h2 class="px-6 py-4 text-base font-semibold text-ink-900">Delivered phone numbers</h2>
                    <div class="table-wrap border-t border-ink-100">
                        <table class="table">
                            <thead><tr><th>Number</th><th>Latest code</th><th>Delivered</th></tr></thead>
                            <tbody>
                                @foreach ($order->phoneNumbers as $number)
                                    <tr>
                                        <td class="font-mono text-xs">{{ $number->phone_number }}</td>
                                        <td class="font-mono text-xs">{{ $number->latest_code ?? '—' }}</td>
                                        <td class="text-sm text-ink-600">{{ $number->delivered_at?->format('d M H:i') ?? '—' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="card p-6">
                ...
                <div class="mt-5 space-y-3">
                    ...
                    @if (in_array($order->status, ['paid', 'failed'], true))
                        ...
                        <p class="text-xs text-ink-500">Buys the {{ match(true) {
                            $order->sku?->isEmailAccount() => 'email account(s)',
                            $order->sku?->isPhoneNumber() => 'phone number(s)',
                            default => 'cloud phone',
                        } }} from VMOS for this order.</p>
                    @endif
